...
static volatile u32 usbkb_not_released;
...
static void usbkb_release_normal_key(int type){
...
void usbkb_hid_report(kb_data_t *data){
	...
	*/
    ...
    static u8 last_normal_key_cnt=0;
    static u8 last_normal_keycode[KEYBOARD_REPORT_KEY_MAX];

    u8 temp_normal_key_cnt = normal_key_cnt;
    u8 temp_normal_keycode[KEYBOARD_REPORT_KEY_MAX];
    memcpy(temp_normal_keycode, normal_keycode, normal_key_cnt);
...
#if 1
	    if(dongle_send_release_flg && last_normal_key_cnt && normal_key_cnt)
	    {
	    	int i,j;
			for(i=0; i< normal_key_cnt; i++)
			{
				for(j=0;j<last_normal_key_cnt;j++)
				{
					if(normal_keycode[i] == last_normal_keycode[j])
					{
						for(int k=i; k<(normal_key_cnt-1); k++){
							normal_keycode[k] = normal_keycode[k+1];
						}

						normal_keycode[normal_key_cnt-1] = 0;
    ...
    last_normal_key_cnt = temp_normal_key_cnt;
    memcpy(last_normal_keycode, temp_normal_keycode, temp_normal_key_cnt);
	dongle_send_release_flg = 0;
