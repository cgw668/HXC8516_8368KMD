...
void OPTSensor_hardware_init(unsigned int *sensor_hw ){
	...
	gpio_setup_up_down_resistor(m_sif_data,  PM_PIN_PULLUP_10K );	//pull up register is 1M
	gpio_setup_up_down_resistor(m_sif_clk, PM_PIN_PULLUP_10K);
...
const SENSOR_OPT Pan_opt[]={
		...
		{Config_M8589, sizeof(Config_M8589)},
		{Config_KA9, sizeof(Config_KA9)},
};
...
#define SensorKA9_Shutdown_Setting()		   DownloadConfigTable(ShutDown_Config_KA9, sizeof(ShutDown_Config_KA9))
#define SensorKA9_WakeUp_Setting()             DownloadConfigTable(WakeUp_Config_KA9, sizeof(WakeUp_Config_KA9))
#define SensorYS8008_Optimization_Setting()    DownloadConfigTable(Config_YS8008, sizeof(Config_YS8008))
#endif
...
u8 mouse_cust_fct3065xy;
extern u32 tick_32k_bk;
...
static inline int sensor_type_identify( unsigned char product_id2 ){
    ...
    if(product_id1 == FCT3065_PRODUCT_ID1 ){
    	if(product_id2 == FCT3065_PRODUCT_NEW_ID2){//fct3065-xy-d
    		mouse_cust_fct3065xy = 1;
    		sensor_type = SENSOR_3205;
    		return sensor_type;
    	}
    	else if(product_id2 == FCT3065_PRODUCT_OLD_ID2 ){//fct3065
    		sensor_type = SENSOR_3205;
    		return sensor_type;
    	}
    }
    ...
    switch( product_id2 &0xF0 ){
        case PAN3204_PRODUCT_ID2://0x50
            ...
            else if(product_id2 == 0x50){			//Identify YS8006
            	u8 reg_17 = I2C_PAN3204LL_ReadRegister(0x17);
//            	u8 reg_18 = I2C_PAN3204LL_ReadRegister(0x18);
//            	u8 reg_19 = I2C_PAN3204LL_ReadRegister(0x19);
//
            	if(reg_17 == 0x5c)
            		sensor_type = SENSOR_YS8006;
            	break;
        ...
        case PAN3204UL_PRODUCT_ID2://0xD0
            ...
            else if(I2C_PAN3204LL_ReadRegister(0x1B) == 0x1D)
            {
            	...
            	if(I2C_PAN3204LL_ReadRegister(0x10) == 0x13 && I2C_PAN3204LL_ReadRegister(0x11) == 0x20){			//add KA9
            		...
            		sensor_type = SENSOR_KA9;//0x10,0x11寄存器在没有写保护读保护的情况下去读，为0x00或0xff
...
*/
		if(sensor_type == SENSOR_YS8008){
			if((reg_x == 0) && (reg_y == 0)&&(device_sleep.mode & M_SUSPEND_100MS))
			{
				reg_x = 1;
				reg_y = 0;
			...
			//if((abs((signed char)reg_x)>dxy)||(abs((signed char)reg_y)>dxy))
			{
				pBuf[0] = (signed char)reg_x/2;
				pBuf[1] = (signed char)reg_y/2;
			}
			else
			{
				pBuf[0] = reg_x;
...
void OPTSensor_Shutdown(void)
{
	...
	for( i=0; i < 16; i++ ){
		...
		if( sensor_type == SENSOR_SIGMA_8630 ){
			...
			I2C_PAN3204LL_WriteRegister(REG_PAN3204LL_CONFIGURATION, 0x0d);
			return;
		}
		else
#endif
...
#if(MOUSE_SENSOR_YS8008_EN)
			if(sensor_type == SENSOR_YS8008){
				return;
			}
			else if(sensor_type == SENSOR_KA9){
...
#if(MOUSE_SENSOR_YS8008_EN)
const unsigned char DPI_convert_table[LAST_SUPPORT_SENSOR] = {3, 0, 1, 1, 1, 4, 0, 3, 3, 1, 0, 3, 0, 0, 0, 0};	//paw3212 = 0x09
#else
...
#endif
u8 spec_cfg_en = 1;
void mouse_sensor_spec_cfg_proc(u8 *cpi , u8 waitdeep){
	int timeout = 0;
	u8  dpi = 0x01;
	u8  dpi_reg;
	u32 cpi_reg = REG_PAN3204LL_CONFIGURATION;

	if((spec_cfg_en && sensor_no_data_cnt >10) || waitdeep){
		if(sensor_type == SENSOR_SIGMA_8630){
			dpi = 0x05;
		}
			...
			dpi =0x33;
			cpi_reg = REG_PAW3212_CPI_X;
		}
#endif

		do{
			I2C_PAN3204LL_WriteRegister(cpi_reg, dpi);
			dpi_reg = I2C_PAN3204LL_ReadRegister(cpi_reg);
		}while( (dpi_reg!=dpi) && (++timeout<16) );
		if(dpi_reg == dpi){
			spec_cfg_en = 0;
		}

	}
	else if((!spec_cfg_en) && (!sensor_no_data_cnt)){
		mouse_sensor_set_cpi ( cpi );
		spec_cfg_en = 1;
	}
