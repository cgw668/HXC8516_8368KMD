...
void  user_init(void)
{
	...
	mouse_button_pull_and_detect(&mouse_status);
...
static inline void mouse_power_saving_process( mouse_status_t *mouse_status ){
...
#if MOUSE_DEEPSLEEP_EN
    device_sleep.quick_sleep = HOST_NO_LINK && QUICK_SLEEP_EN && mouse_data.btn != 0x03 && mouse_data.btn != 0x07 && mouse_data.btn != 0x06 ;
    ...
    if(( data_check ) && ( M_SUSPEND_MCU_SLP & device_sleep.mode )){// wait deep
    	device_sleep.mode = 4;
    	device_sleep.sensor_sleep = 0;
    }

    mouse_sensor_spec_cfg_proc(&mouse_status->cpi , (device_sleep.mode & M_SUSPEND_MCU_SLP));
	...
	else if(device_sleep.mode == 4 ){  // wait deep
		device_sleep.wakeup_time =  wait_deep_time;
...
void mouse_task(u8 wheel_prepare_tick){
...
#endif
        data_check = (*((u32 *)(mouse_status.data))) ? 1 : 0;
        mouse_task_in_ram();
	}
}
