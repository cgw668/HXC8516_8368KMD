...
typedef struct{
	...
	mouse_hw_t cust_ms_hw; 	//0x4c mouse hw_define

#if (CAVY_HAMSTER_REMINGTON_COMPATIBLE_EN)
	u8 chn_type;			//0x98, 0 for cavy chn, ff for hamster chn
#endif
	u8 wait_deep_time;		//0x99
#if 0
typedef struct {
...
extern custom_cpi_cfg_t mouse_4_sgmt_cpi;
...
#define mcu_restart_en      		(p_custom_cfg->restart_en != U8_MAX)
#define wait_deep_time				(p_custom_cfg->wait_deep_time == U8_MAX ? 100: p_custom_cfg->wait_deep_time)
...
#define batt_low_ref_s 		(p_custom_cfg->cust_ms_hw.vbat_low_ref_s == 0xff ? 0x30 : p_custom_cfg->cust_ms_hw.vbat_low_ref_s)//V1P0_REF
#define batt_low_scale_s	(p_custom_cfg->cust_ms_hw.vbat_low_scale_s == 0xff ? 0x60 : p_custom_cfg->cust_ms_hw.vbat_low_scale_s)//V1P0_SCALE
#define batt_low_ref_d 		(p_custom_cfg->cust_ms_hw.vbat_low_ref_d == 0xff ? 0x30 : p_custom_cfg->cust_ms_hw.vbat_low_ref_d)//V2P0_REF
#define batt_low_scale_d 	(p_custom_cfg->cust_ms_hw.vbat_low_scale_d == 0xff ? 0x20 : p_custom_cfg->cust_ms_hw.vbat_low_scale_d)//V2P0_SCALE
