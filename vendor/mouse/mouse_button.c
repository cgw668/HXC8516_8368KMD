...
u32 mouse_button_process(mouse_status_t * mouse_status)
{
    ...
    static u16 btn_lrm_cnt = 0;
    static u16 btn_mr_cnt = 0;

    if ( button_pre == button_last ){
        if( button == 0x03 )//right & left
            btn_lr_cnt++;
        else
            btn_lr_cnt = 0;
        if( button == 0x06 )//right & middle
            btn_mr_cnt++;
        else
            btn_mr_cnt = 0;
    ...
    if (button_pre != button) {            //new event
		...
		if( (mouse_status->mouse_mode == STATE_POWERON)){
			if((button == 0x03)){
				test_mode_pending = STATE_PAIRING;
			}
			else if((button == 0x07)){
				test_mode_pending = STATE_EMI;
			}
		}
		if(test_mode_pending && (!button)  ){
			if(test_mode_pending == STATE_PAIRING) {
				mouse_status->mouse_mode = STATE_PAIRING;
			}
			else if(test_mode_pending == STATE_EMI){
				mouse_status->mouse_mode = STATE_EMI;
			}
			test_mode_pending = 0;
		}

        if((btn_mr_cnt > 300) && mcu_restart_en){
        	btn_mr_cnt = 0;
