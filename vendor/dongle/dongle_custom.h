...
typedef struct{
	...
	u8   channal_msk;      // 09
	//1): USB polling interval 2ms;
	//0): USB polling interval 4ms, default;
	u8 	 report_type;      // 0a
	u16  vendor_id;        // 0b-0c
	u16  prodct_id;        // 0d-0e
	u8   tx_power_emi;     // 0f
	...
	u8   usb_report_type;  //Interpolate mouse data;
...
#define dongle_interpolate_en			 (p_custom_cfg->usb_report_type != 0xff)
