...
void	callback_pairing (u8 * p)
{
	...
	int  device_paring_flag;
	int  device_power_on_paring;
...
//    rx_rssi = RECV_PKT_RSSI(p);
	...
	if(work_with_cavy_mouse){
		device_paring_flag = (((!(p_pkt->flow & 0x80)) && (p_pkt->flow & PKT_FLOW_PARING )) || (p_pkt->flow & 0x08));                          //有PKT_FLOW_PARING 标志的为paring包，否则是link包
		device_power_on_paring = p_pkt->flow & PKT_FLOW_TOKEN;                             //有PKT_FLOW_TOKEN 标志的，是上电配对包, manual在已经有配对device后，不响应上电配对
		cavy_rssi_pair = 1;
	}
	else{
		device_paring_flag     = p_pkt->flow & PKT_FLOW_PARING;                          //有PKT_FLOW_PARING 标志的为paring包，否则是link包
		device_power_on_paring = p_pkt->flow & PKT_FLOW_TOKEN;                             //有PKT_FLOW_TOKEN 标志的，是上电配对包, manual在已经有配对device后，不响应上电配对
	}

	if(auto_paring_enable && device_not_pair &&(cavy_rssi_pair || (rx_rssi > (30 - custom_rssi_paring_th_offset)))) {  //auto paring只在没有配对过的情况下响应 -82dbm threshold
	...
	if( manual_paring_enable && rf_paring_enable   &&  device_paring_flag \
	    && (cavy_rssi_pair || rssi_paring_good)  && !(!device_not_pair &&  device_power_on_paring) ){
...
void	callback_mouse (u8 *p)
{
	...
	if (p_pkt->seq_no != seq_no_mouse && reg_usb_host_conn) {	//skip same packet
		...
		usbmouse_add_frame(p_pkt, dongle_interpolate_en);
...
void debug_mouse (void) {
	...
	if (reg_usb_host_conn && clock_time_exceed (tick_m, 8000)) {
        ...
        pkt_mouse.seq_no++;
        usbmouse_add_frame(&pkt_mouse,0);
	}
}
