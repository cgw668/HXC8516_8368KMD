...
enum{
	 ...
	 WAKEUP_SRC_TIMER = BIT(1),
	 WAKEUP_SRC_CORE = BIT(2),
	 WAKEUP_SRC_PAD = BIT(3),
};
...
#define		V0P9_REF			REF_VOLTAGE_SEL_910_MV
#define		V0P9_SCALE			SCALING_SELECT_FULL
...
#define		V1P9_REF			REF_VOLTAGE_SEL_955_MV	//1.91v
#define		V1P9_SCALE			SCALING_SELECT_HALF
...
static inline u32 battery_low_detect_auto ( u8 vbat_chn , u8 low_ref_s , u8 low_scale_s , u8 low_ref_d , u8 low_scale_d  ){
    if ( vbat_chn > COMP_AVDD )     //no detect
        return 0;
    
    int bat_low_flg = 0;
    if ( battery_low_by_set (vbat_chn, VBAT_LOW_REF_MIDL, VBAT_LOW_SCALE_MIDL) ){
        bat_low_flg = battery_low_by_set (vbat_chn, low_ref_s, low_scale_s) ;
    }
    else{
        bat_low_flg = battery_low_by_set (vbat_chn, low_ref_d, low_scale_d) ;
    }
    return bat_low_flg;
}
