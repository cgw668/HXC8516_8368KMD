...
#if(__PROJECT_MOUSE__ ||__PROJECT_8366_MOUSE_MJ__| __PROJECT_KEYBOARD__ || __PROJECT_8366_MIIW_MOUSE__)
...
#define tms 	wakeup_ms
	...
	u8 core_61;
	if(!deepsleep){
		ana_84 = analog_read(0x84);
		analog_write(0x84,ana_84 | 0x01);
		core_61 = read_reg8(0x61);
		write_reg8(0x61, core_61 | 0x80);
	}
#endif
	analog_write (0x17, wakeup_src | PM_AUTO_PWDN_EN);
	analog_write (0x23, 0x0f);  //clear status
...
#if (__PROJECT_MOUSE__)
	if (!deepsleep) {
		analog_write(0x84,ana_84);
		write_reg8(0x61, core_61);
	}
#endif
	//////////////////// restore GPIO wake-up configuration ////////////////
	if (! (wakeup_src & PM_WAKEUP_PAD)) {
